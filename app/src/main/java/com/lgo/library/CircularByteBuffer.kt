...
class CircularByteBuffer {
     ...
     * @since ostermillerutils 1.00.00
     */
    protected var `in`: InputStream = CircularByteBufferInputStream()
    /**
     ...
     * @since ostermillerutils 1.00.00
     */
    protected var out: OutputStream = CircularByteBufferOutputStream()
    /**
     ...
     */
    constructor() {
        CircularByteBuffer(DEFAULT_SIZE, true)
    }
     ...
     * @since ostermillerutils 1.00.00
     */
    constructor(blockingWrite: Boolean) {
        CircularByteBuffer(DEFAULT_SIZE, blockingWrite)
    }
     ...
     * @since ostermillerutils 1.00.00
     */
    constructor(size: Int, blockingWrite: Boolean) {
        ...
        override fun read(): Int {
            while (true) {
                synchronized(this@CircularByteBuffer) {
                    ...
                    if (available > 0) {
                        val result = buffer!![readPosition] and (0xff).toByte()
                        readPosition++
            ...
            while (true) {
                synchronized(this@CircularByteBuffer) {
                    ...
                    if (available > 0) {
                        ...
                        val firstLen = Math.min(length, buffer!!.size - readPosition)
        ...
        override fun write(cbuf: ByteArray, off: Int, len: Int) {
            ...
            while (len > 0) {
                synchronized(this@CircularByteBuffer) {
                    ...
                    val firstLen = Math.min(realLen, buffer!!.size - writePosition)
                    val secondLen = Math.min(realLen - firstLen, buffer!!.size - markPosition - 1)
